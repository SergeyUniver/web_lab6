...
public class SignInAction extends Action implements IAction {
    ...
    public SignInAction(DAOCreator daoCreator) {
        super(daoCreator);
    }
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            User user = daoCreator.getUserDAO().tryAuthorize(login, password);
            if (user == null)
            {
                HttpSession session = request.getSession();
                session.setAttribute(MESSAGE_PARAM, "Wrong login or password");
